...
async def call_llamacpp_async(session, messages):
    ...
    try:
        async with session.post(LLAMA_CPP_URL, headers=headers, json=payload) as resp:
            if resp.status == 200:
                result = await resp.json()
                try:
                    return result['choices'][0]['message']['content']
                except (KeyError, IndexError) as e:
                    print("Unexpected llama.cpp response structure:", result)
                    return None
            else:
                text = await resp.text()
                print(f"llama.cpp API error: {resp.status} - {text}")
                return None
    except Exception as e:
        print("Error calling llama.cpp:", e)
        return None
...
async def generate_search_queries_async(session, user_query):
    ...
    if response:
        try:
            regex = r"\{.*\}"
            if json_match := re.search(regex, response, re.DOTALL):
                 json_str = json_match.group()
                 search_queries = json.loads(json_str).get("queries", [])
...
async def extract_relevant_context_async(session, user_query, search_query, page_text):
    ...
    prompt = (
        ...
        "Return only the relevant context as a JSON precisely in the following format: "
        '{ "response" : <>}, where <> must be substituted by a string with a plain text without commentaries.'
    )
    messages = [
